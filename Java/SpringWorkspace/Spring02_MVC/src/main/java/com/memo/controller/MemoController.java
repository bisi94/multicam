...
public class MemoController { //POJO
	...
	public String memoInsert(Model model, @ModelAttribute("vo") MemoVO vo) {
		//@ModelAttribute를 붙이면 html의 input name하고 VO객체의 property명이 같을 경우 
		//자동으로 입력값을 VO에 setting해준다
		...
		if(vo.getName()==null|| vo.getName().trim().isEmpty()
			...
			//뷰네임 앞에 "redirect:"을 붙여주면 redirect방식으로 이동한다
		}
	...
	//매핑하고 뷰네임 반환하기
	@RequestMapping(value="/memoList") //default가 GET방식임
	public String memoList(Model model, @RequestParam(name="cpage", defaultValue="1") int cpage) {
		...
		//List<MemoVO>객체를 model에 저장하기. 키값: memoArr
