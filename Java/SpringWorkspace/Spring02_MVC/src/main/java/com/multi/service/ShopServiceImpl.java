...
public class ShopServiceImpl implements ShopService {
	...
	public int addCart(CartVO cartVo) {
		...
		// 2. 담겨있지 않은 경우 (insert문)
		//---------------------------------------------------
		// [1] userid와 pnum_fk로 cart테이블에 있는 상품개수 가져오기
		Integer cnt=cartMapper.selectCartCountByPnum(cartVo);
		System.out.println("cnt: "+cnt);
		// [1_1] 개수가 >0 => 이미 담긴 상품 => 수량만 수정
		if(cnt!=null) {
	...
	public int editCart(CartVO cartVo) {
		...
		if(qty==0) {
			...
			throw new NumberFormatException("수량은 0보다 커야 합니다");
		}else if(qty>50) {
			throw new NumberFormatException("수량은 50개 이내로 수정 가능합니다");
		}else { //유효한 수량값이라면
