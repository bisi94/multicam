...
<div class="container mt-5" id="content">
	...
	${loginUser.name} [${loginUser.userid}]님의 장바구니</h2>
	<br><br>
			...
			<c:if test="${cartArr ne null and not empty cartArr}">
				<c:forEach var="item" items="${cartArr}" varStatus="state">
					...
					<tr>
						...
						id="pnum${state.index}" value="${item.pnum_fk}">
						${item.pnum_fk}
						</label>
						</td>
						<td>
						...
						<td><fmt:formatNumber value="${item.saleprice}" pattern="###,###" /> 원<br>
							<span class="badge badge-danger">${item.point}</span> POINT</td>
						<td><fmt:formatNumber value="${item.totalPrice}" pattern="###,###" /> 원<br>
							<span class="badge badge-danger">${item.totalPoint}</span> POINT</td>
						<td><a href="#" onclick="cartDel('${item.cartNum}')">삭제</a></td>
					</tr>
				</c:forEach>
			</c:if>
				<!-- --------------------------- -->
				...
				<tr>
					<td colspan="3">
						<h5>
							장바구니 총액: <span class='text-danger'> 
...
<script>
	...
	function cartDelAll(cnum){//장바구니 모두 비우기
