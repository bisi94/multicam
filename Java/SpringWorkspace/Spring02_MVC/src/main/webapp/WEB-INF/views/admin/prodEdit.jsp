			...
			<table class="table table-condensed mt-4">
					...
					<td width="80%">
						<select name="upCg_code" id="upCg_code">
							...
							<c:forEach var="up" items="${upCgList}">
								<option value="${up.upCg_code}" 
									<c:if test="${item.upCg_code eq up.upCg_code}">selected</c:if>
								>
									${up.upCg_name}
								</option>
							</c:forEach>
						...
						<span id="downCg">
							<select name="downCg_code" id="downCg_code">
								...
								<!-- ------------------------------ -->
								<option value="1" <c:if test="${item.downCg_code eq 1}">selected</c:if> >컴퓨터</option>
								<option value="2" <c:if test="${item.downCg_code eq 2}">selected</c:if> >가전제품</option>
								<!-- ------------------------------ -->
							</select>	
				...
				<tr>
					...
					<td width="80%">
						...
						<br>
						<input type="file" name="pimage" accept="iamges/*" class="form-control"><br>
						<input type="file" name="pimage" accept="iamges/*" class="form-control"><br>
						<input type="file" name="pimage" accept="iamges/*" class="form-control"><br>
					</td>
				</tr>
